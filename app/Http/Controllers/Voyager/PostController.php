...
class PostController extends VoyagerBaseController
{
    ...
    public function delete_value($id, $field)
    {
        ...
        // dd($post);
        if (isset($post->id)) {
            if (Storage::disk(config('voyager.storage.disk'))->exists($post->$field)) {
                Storage::disk(config('voyager.storage.disk'))->delete($post->$field);
            }
            $post->$field = '';
            $post->save();
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $post = Post::find($id);
        $categoriesForPost = $post->categories()->get();

        //dd($categoriesForPost);
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'allCategories', 'categoriesForPost'));
    }
