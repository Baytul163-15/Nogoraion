...
<!-- ///////////////////////////1st section starts -->
            ...
            <div class="row  former_chief_wp">
                ...
                @php
                $sl = $fmdata->perPage() * ($fmdata->currentPage() - 1) +1;
                @endphp
                @foreach($fmdata as $key => $fdata)

                <div class="col-md-12">
                    <div class="notice-img">
                        <img src="{{ url('public/storage/'.$fdata->image)}}" alt="{{ $fdata->title }}" style="width: 300px;height: 300px;float: left;">
                    </div>
                    {!! $fdata->body !!} 
                </div>
                @if(($key+1) % 3 == 0)
            ...
            <div class=" full-height text-center">
                ...
                @auth
                <p>Please Post in this category. Category slug must be : {{ $slug }} </p>
                @endauth
